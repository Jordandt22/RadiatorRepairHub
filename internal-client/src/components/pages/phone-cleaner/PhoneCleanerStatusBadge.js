...
import { PHONE_STATUS_LABELS } from "@/components/pages/phone-cleaner/PhoneCleanerMarkStatusDialog";
...
export default function PhoneCleanerStatusBadge({ status }) {
  const key = status && STYLES[status] ? status : "not_checked";
  const style = STYLES[key];
  const Icon = style.icon;
  const label = PHONE_STATUS_LABELS[key] ?? "Not Checked";
