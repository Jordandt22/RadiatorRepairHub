...
export default function PhoneCleanerMarkStatusDialog({
  open,
  onOpenChange,
  selectedCount = 0,
  initialStatus = null,
  ...
  useEffect(() => {
    if (open) {
      const match = initialStatus
        ? PHONE_STATUS_OPTIONS.find((option) => option.id === initialStatus)
        : null;
    ...
    >
      <DialogContent className="sm:max-w-md" showCloseButton={!confirmPending}>
        <DialogHeader>
          ...
          <DialogDescription>
            Set the phone review status for {countLabel}.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-2 py-1">
          ...
          <BusinessTierCombobox
            ...
            ariaLabel="Phone review status"
            inputName="rrh-phone-cleaner-mark-status"
