...
const OUTCOME_COLUMNS = [
  ...
  },
  {
    ...
    label: "Status",
    className: "w-[14%]",
    render: (row) => <UploadPhotosStatusBadge status={row.status} />,
  },
  {
    key: "image_id",
    label: "Image ID",
    className: "w-[20%]",
    getValue: (row) => row.image_id || "—",
  },
  {
    key: "error",
    label: "Error",
  ...
  },
  {
    ...
    render: (row) => (
      <Button
        ...
        nativeButton={false}
        render={<Link href={`/upload-photos/batch/${row.id}`} />}
      >
        <EyeIcon />
        View
      </Button>
...
export default function UploadPhotosJobDetailPageContent() {
  ...
  const detailQuery = useQuery({
    ...
    queryFn: async () => {
      const result = await fetchApi(`/admin/cdn-upload/jobs/${jobId}`, {
        accessToken,
      });
    ...
    return (
      <div className="mx-auto flex w-full flex-1 flex-col gap-3 px-4 py-4 md:gap-4 md:px-8 md:py-6">
        <Button
          ...
          nativeButton={false}
          render={<Link href="/upload-photos?tab=jobs" />}
        >
          <ArrowLeftIcon />
          Back to jobs
        </Button>
        <p className="text-sm text-destructive">
          {detailQuery.error?.message || "CDN upload job not found."}
        </p>
      </div>
    );
  }
    ...
    <div className="mx-auto flex w-full flex-1 flex-col gap-4 px-4 py-4 md:gap-5 md:px-8 md:py-6">
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <h1 className="text-xl font-semibold tracking-tight">Upload job</h1>
          <UploadPhotosStatusBadge status={job.status} />
        </div>
