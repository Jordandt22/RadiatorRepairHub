...
export const LOCATION_DETAIL_TABS = [
  ...
  { id: "search-demand", label: "Search demand", Icon: SearchIcon },
  { id: "analytics", label: "Analytics", Icon: BarChart3Icon },
];

export default function LocationDetailTabs({ value, onValueChange }) {
  const triggerClassName =
    "cursor-pointer hover:translate-y-[-2px] transition-all duration-200 px-6 rounded-full md:px-8";
  const activeTab =
    LOCATION_DETAIL_TABS.find((tab) => tab.id === value) ??
        ...
        <DropdownMenu>
          ...
          >
            <DropdownMenuRadioGroup value={value} onValueChange={onValueChange}>
              {LOCATION_DETAIL_TABS.map(({ id, label, Icon }) => (
                <DropdownMenuRadioItem
                  key={id}
                  value={id}
