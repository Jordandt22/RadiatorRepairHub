ALTER TABLE public.business_stats
  ADD COLUMN IF NOT EXISTS impressions_nearby integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS nearby_position_sum integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS listing_clicks_nearby integer NOT NULL DEFAULT 0;

DO $$ BEGIN
  ALTER TABLE public.business_stats
    ADD CONSTRAINT business_stats_impressions_nearby_check
    CHECK (impressions_nearby >= 0);
EXCEPTION
...
DO $$ BEGIN
  ALTER TABLE public.business_stats
    ADD CONSTRAINT business_stats_nearby_position_sum_check
    CHECK (nearby_position_sum >= 0);
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  ALTER TABLE public.business_stats
    ADD CONSTRAINT business_stats_listing_clicks_nearby_check
    CHECK (listing_clicks_nearby >= 0);
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;
