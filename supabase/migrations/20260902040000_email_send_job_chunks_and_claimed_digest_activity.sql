-- Allow multiple send jobs per campaign/segment (50-email chunks) and
-- require recent activity for claimed weekly digests at reservation time.
...
ALTER TABLE public.digest_send_jobs
  DROP CONSTRAINT IF EXISTS digest_send_jobs_run_segment_key;

ALTER TABLE public.outreach_send_jobs
  DROP CONSTRAINT IF EXISTS outreach_send_jobs_run_type_key;

ALTER TABLE public.digest_send_jobs
  DROP CONSTRAINT IF EXISTS digest_send_jobs_chunk_check;

ALTER TABLE public.digest_send_jobs
  ADD CONSTRAINT digest_send_jobs_chunk_check
  CHECK (chunk_index >= 0);

ALTER TABLE public.outreach_send_jobs
  DROP CONSTRAINT IF EXISTS outreach_send_jobs_chunk_check;

ALTER TABLE public.outreach_send_jobs
  ADD CONSTRAINT outreach_send_jobs_chunk_check
  CHECK (chunk_index >= 0);

ALTER TABLE public.digest_send_jobs
  ADD CONSTRAINT digest_send_jobs_run_segment_chunk_key
  UNIQUE (run_id, digest_segment, chunk_index);

ALTER TABLE public.outreach_send_jobs
  ADD CONSTRAINT outreach_send_jobs_run_type_chunk_key
  UNIQUE (run_id, outreach_type, chunk_index);
