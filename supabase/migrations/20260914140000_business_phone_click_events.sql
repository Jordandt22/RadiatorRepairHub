CREATE TABLE IF NOT EXISTS public.business_phone_click_events (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  business_id uuid NOT NULL REFERENCES public.businesses(id) ON DELETE CASCADE,
  created_at timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_business_phone_click_events_business_id_created_at
  ON public.business_phone_click_events USING btree (business_id, created_at DESC);

ALTER TABLE public.business_phone_click_events ENABLE ROW LEVEL SECURITY;

GRANT ALL ON TABLE public.business_phone_click_events TO anon;
GRANT ALL ON TABLE public.business_phone_click_events TO authenticated;
GRANT ALL ON TABLE public.business_phone_click_events TO service_role;

DO $$ BEGIN
  CREATE POLICY "Owners can read business phone click events"
    ON public.business_phone_click_events
    FOR SELECT
    TO authenticated
    USING (
      EXISTS (
        ...
        WHERE b.id = business_phone_click_events.business_id
...
BEGIN
  ...
  IF p_event = 'listing_click' THEN
    ...
    ELSIF p_source = 'category' THEN
      ...
      SET listing_clicks = listing_clicks + 1,
          ...
          updated_at = now()
      WHERE business_id = p_business_id AND stat_date = p_stat_date;
    ELSIF p_source = 'nearby' THEN
      UPDATE public.business_stats
      SET listing_clicks = listing_clicks + 1,
          listing_clicks_nearby = listing_clicks_nearby + 1,
          updated_at = now()
      WHERE business_id = p_business_id AND stat_date = p_stat_date;
    ELSE
  ...
  IF p_event = 'phone_click' THEN
    ...
    INSERT INTO public.business_phone_click_events (business_id)
  ...
  IF p_event = 'impression' THEN
    ...
    ELSIF p_source = 'nearby' THEN
      UPDATE public.business_stats
      SET impressions_nearby = impressions_nearby + 1,
          nearby_position_sum = nearby_position_sum + p_position,
          updated_at = now()
      WHERE business_id = p_business_id AND stat_date = p_stat_date;
    ELSE
