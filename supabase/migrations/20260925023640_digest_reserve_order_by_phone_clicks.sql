-- Prefer weekly digest recipients with the most phone clicks first when a
-- send job has a limited batch size, then fall back to impressions.
...
BEGIN
  ...
  IF v_needed > 0 THEN
    ...
    ORDER BY
      COALESCE((
        SELECT SUM(COALESCE(stats.phone_clicks, 0))
        FROM public.business_stats AS stats
        WHERE stats.business_id = business.id
          AND stats.stat_date >= v_activity_start
      ), 0) DESC,
