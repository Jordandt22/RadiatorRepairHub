-- Listing analytics: Has website / No website filter on list + summary RPCs.
...
CREATE FUNCTION public.admin_list_business_stats(
  ...
  p_website_filter text DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
...
DECLARE
  ...
  v_website_filter text := p_website_filter;
  v_offset integer;
  v_result jsonb;
BEGIN
  ...
  IF v_website_filter IS NOT NULL AND v_website_filter NOT IN ('has', 'none') THEN
    v_website_filter := NULL;
  END IF;
  ...
  filtered AS (
    ...
    WHERE COALESCE(b.is_test, false) = false
      ...
      AND (
        v_website_filter IS NULL
        OR (v_website_filter = 'has' AND b.website IS NOT NULL AND b.website <> '')
        OR (v_website_filter = 'none' AND (b.website IS NULL OR b.website = ''))
      )
      AND (
        p_q IS NULL
...
REVOKE EXECUTE ON FUNCTION public.admin_list_business_stats(
  date, date, text, boolean, boolean, text, text, integer, integer, uuid, uuid, text, text, text
...
REVOKE EXECUTE ON FUNCTION public.admin_summary_business_stats(
  date, date, boolean, boolean, uuid, uuid, text, text, text
) FROM PUBLIC, anon, authenticated;
