...
const attachmentVariants = cva(
  "group/attachment relative flex w-full min-w-0 shrink-0 flex-wrap rounded-lg border border-border bg-card text-card-foreground transition-colors focus-within:ring-1 focus-within:ring-ring/50 has-[>a,>button]:hover:bg-muted/50 data-[state=error]:border-destructive/30 data-[state=idle]:border-dashed",
  {
    variants: {
      size: {
        default:
...
function AttachmentGroup({ className, ...props }) {
  return (
    <div
      ...
      className={cn(
        "flex min-w-0 snap-x snap-mandatory flex-col gap-2 overflow-y-auto",
        className
      )}
      {...props}
    />
