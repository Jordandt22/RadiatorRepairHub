...
export const getCountBusinessesBySearchKey = (
  ...
  key: createSearchBusinessKey(
    `SEARCHED_BUSINESSES_COUNT?ORD:featured2&SORT-OPTION:${sort_option}`,
    searchParamValues
  ),
  interval: 60 * 30,
});
...
export const getSearchedBusinessesKey = (
  ...
  key: createSearchBusinessKey(
    `SEARCHED_BUSINESSES?ORD:featured2&PAGE:${page}&LIMIT:${limit}&SORT-OPTION:${sort_option}`,
    searchParamValues
  ),
  interval: 60 * 30,
});
...
export const getListingReportsKey = (page, limit, status = null) => ({
  key: `LISTING_REPORTS?PAGE:${page}&LIMIT:${limit}&STATUS:${status ?? "all"}`,
  interval: 60 * 5,
});
